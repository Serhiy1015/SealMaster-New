...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (navList) {
    ...
    li.querySelector('button').addEventListener('click', () => {
      const target = document.querySelector('#homeDimFilterSeal') || document.querySelector('#dim-search');
      if (target) {
  ...
  return lines.slice(1)
    ...
    .map(row => {
      ...
      if (obj.image && !obj.image.startsWith('http') && !obj.image.startsWith('images/')) {
        obj.image = `https://lh3.googleusercontent.com/d/${obj.image}=w800`;
      }
      return obj;
    });
}
...
async function loadProducts() {
  ...
  const CACHE_KEY = 'products_csv_v10';
